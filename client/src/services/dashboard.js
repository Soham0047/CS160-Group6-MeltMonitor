...
// Store data in database (optional - only when backend is running)
async function storeData(co2, temp) {
  ...
  try {
    ...
    // Silently ignore - backend is optional for frontend functionality
    // Only log in development if needed for debugging
    if (e.name !== "AbortError") {
      console.debug("Backend unavailable (optional):", e.message);
    }
  }
}
